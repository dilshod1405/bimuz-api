...
class BookingAPITestCase(TestCase):
    ...
    def test_book_student_old_group_fails_10_day_rule(self):
        ...
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        # Response can be DRF validation error (field names as keys) or error_response format
        if isinstance(response.data, dict):
            ...
            # In DRF validation errors, errors are directly in response.data (field names as keys)
            # In error_response format, errors are in response.data['errors']
            errors = response.data.get('errors', response.data) if 'errors' in response.data else response.data
            # Extract all error messages
            all_error_messages = []
            for field, error_list in errors.items():
                if isinstance(error_list, list):
                    all_error_messages.extend([str(err).lower() for err in error_list])
                else:
                    all_error_messages.append(str(error_list).lower())
            ...
            has_10_day_rule = (
                ...
                any('10 kunlik' in msg or 'cheklov' in msg for msg in all_error_messages)
            )
    ...
    def test_book_student_full_group_fails(self):
        ...
        if isinstance(response.data, dict):
            ...
            all_error_messages = []
            for field, error_list in errors.items():
                if field in ['success', 'message', 'data']:
                    continue
                if isinstance(error_list, list):
                    all_error_messages.extend([str(err).lower() for err in error_list])
                else:
            ...
            has_seat_error = (
                ...
                any('o\'rin' in msg or 'to\'liq' in msg for msg in all_error_messages)
            )
    ...
    def test_book_student_already_booked_fails(self):
        ...
        if isinstance(response.data, dict):
            ...
            has_already_booked_error = (
                ...
                any('yozilgan' in msg for msg in all_error_messages)
            )
